module axi4_lite_slave (
    ...
    axi_uart_intf.axi_slave axi_slave_port
    
);

    // Register bank - 16 x 32-bit registers
    logic [31:0] register_bank [0:15];
    // 0x00 is the status register
    // 0x01 is the TX DATA register
    // 0x02 is the BAUD RATE register

    logic          addr_valid_write_1;
    // Address decode
    ...
    always_comb begin
        ...
        addr_valid_write_1 = (wr_addr[31:6] == 26'h0) && (wr_addr[1:0] == 2'b00) && (wr_addr[5:0] >= 6'h00 && wr_addr[5:0] <= 6'h3C);
        write_addr_index = wr_addr[5:2];
        not_read_only_reg = (write_addr_index != 4'd0) && (write_addr_index != 4'd3); // Register 0 and 3 are read-only
        addr_valid_write = addr_valid_write_1 && not_read_only_reg;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        ...
        if (!rst_n) begin
        for (int i = 1; i < 16; i++) begin  // Skip register 0 (status reg)
    ...
    always_comb begin
        ...
        else if (rd_data_en  && addr_valid_read) begin
            axi_if.rdata = register_bank[read_addr_index];
            case (read_addr_index)
                4'd0: axi_if.rdata = status_reg;                        // Read-only status register
                4'd3: axi_if.rdata = rx_data_reg;                       // Read-only RX data register
                default: axi_if.rdata = register_bank[read_addr_index]; // Writable registers
            endcase
        end
        else begin
    ...
    //------------------------------------------------------------
    always_comb begin
        status_reg              = 32'd0;                    // STATUS REG 0x00 read only
        status_reg[0]           = axi_slave_port.tx_ready;  // bit 0 -> tx_ready status
        status_reg[1]           = axi_slave_port.rx_ready;  // bit 1 -> rx_ready status
        status_reg[2]           = axi_slave_port.frame_error;// bit 2 -> frame error

        
        axi_slave_port.tx_data      = register_bank[4'd1];      // REG 0x01  = TX DATA
        axi_slave_port.tx_baud_rate = register_bank[4'd2];      // REG 0x02  = tx_BAUD RATE
        axi_slave_port.rx_baud_rate = register_bank[4'd4];      // REG 0x04  = rx_BAUD RATE

    end
    //-------------- TX_VALID LOGIC-------------------------
    always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        axi_slave_port.tx_valid <= 0;
    end
    else if (axi_slave_port.tx_ack) begin
        axi_slave_port.tx_valid <= 0;
    end
    else if (wr_data_en && addr_valid_write && write_addr_index == 4'd1) begin
        axi_slave_port.tx_valid <= 1;
    end
    else begin
        axi_slave_port.tx_valid <= axi_slave_port.tx_valid;
    end
    end
